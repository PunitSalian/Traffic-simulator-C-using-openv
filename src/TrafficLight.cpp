...
T MessageQueue<T>::receive()
{
    ...
    // The received object should then be ret    std::this_thread::sleep_for(std::chrono::milliseconds(100));        // perform vector modification under the lock
    ...
    return currphase; // will not be copied due to return value optimization (RVO) in C++urned by the receive function. 
}
